...
export class ModelScene extends Scene {
  ...
  targetDamperX = new Damper();
  targetDamperY = new Damper();
  targetDamperZ = new Damper();

  _currentGLTF = null;
  _model = null;
  mixer;
  cancelPendingSourceChange = null;
      ...
      const encoding = [
        {
          '@type': 'MediaObject',
          contentUrl: src,
          encodingFormat: src.split('.').pop()?.toLowerCase() === 'gltf' ? 'model/gltf+json' : 'model/gltf-binary',
        },
      ];
